...
function SingleOrderReturned() {
  ...
  const singleReturnedOrder = useSelector(
    (state) => state.order?.single_returned_order?.data?.order_item
  );
  const Returningamount = useSelector(
    (state) => state.order?.single_returned_order?.data
  );

  console.log(singleReturnedOrder, "singleReturnedOrder");
  ...
  useEffect(() => {
    if (
      ...
      dispatch(getSingleReturnOrder(formData))
        .then((response) => {
    ...
    dispatch(pickedupOrder(formData))
      .then((response) => {
        if (response.payload.success) {
          ...
          form_data.append("token", localStorage.getItem("auth_token"));
          form_data.append("user_id", localStorage.getItem("user_id"));
          dispatch(getSingleOrder(form_data))
            .then((response) => {
              if (response.payload.success) {
                setLoading(false);
                window.location.replace("/home");
  ...
  const DeliveredOrder = (orderId) => {
    ...
    formData.append("user_id", localStorage.getItem("user_id"));
    formData.append("order_id", orderId);
    dispatch(orderDelivered(formData))
      .then((response) => {
        if (response.payload.success) {
          const form_data = new FormData();
          form_data.append("token", localStorage.getItem("auth_token"));
  ...
  return (
    <div style={{ minHeight: "100vh" }} className="bg-slate-50">
      ...
      <div className="px-3 w-100">
        ...
        >
          <div style={{ color: "#070648", fontWeight: "700" }} className="px-3">
            #{singleReturnedOrder?.order?.unique_order_id}
          </div>
          ...
          <div className="leading-loose text-muted px-3">
            <div className="flex justify-between">
              ...
              <div className="font-bold">
                {singleReturnedOrder?.order?.user_name}
              </div>
            </div>
            <div className="flex justify-between">
              <div className="font-medium">Customer number</div>
              <div className="font-bold ">
                {singleReturnedOrder?.order?.user?.phone}
              </div>
            </div>
            <div className="flex justify-between">
              <div className="font-medium">Address</div>
              <div className="w-75 font-bold text-end">
                {singleReturnedOrder?.order?.address}
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 px-3">
            <a
              className="order-cancel-button text-center mt-3 flex items-center justify-center no-underline"
              href={`https://maps.google.com/maps?q=${singleReturnedOrder?.order?.latitude},${singleReturnedOrder?.order?.longitude}`}
              target={"_blank"}
            >
              <LiaDirectionsSolid color="#118A45" size={25} />{" "}
              <span className="ms-1">Direction</span>
            </a>
            {singleReturnedOrder?.order?.user?.phone ? (
              <a
                href={`tel:${singleReturnedOrder?.order?.user?.phone}`}
                className="order-accept-button text-center mt-3 flex items-center justify-center no-underline"
              >
                <CiMobile2 color="#fff" size={24} />{" "}
                <span className="ms-1">Call</span>
              </a>
            ) : (
              <a
                className="order-accept-button-disable text-center mt-3 flex items-center justify-center no-underline"
                title="Number Not Giver Sorry"
              >
                <CiMobile2 color="#fff" size={24} />{" "}
                <span className="ms-1">Call</span>
              </a>
        ...
        >
          ...
          <div className="leading-loose text-muted px-3">
            <div className="flex justify-between">
              ...
              <div className="font-bold">
                {singleReturnedOrder?.order?.warehouse?.phone}
              </div>
            </div>
            <div className="flex justify-between">
              <div className="font-medium">address</div>
              <div className="w-75 font-bold text-end">
                {singleReturnedOrder?.order?.warehouse?.address}
              </div>
            </div>
            {/* <div className='flex justify-between'>
          ...
          <div className="flex gap-3 px-3">
            <a
              className="direction-button text-center mt-3 flex items-center justify-center no-underline"
              href={`https://maps.google.com/maps?q=${singleReturnedOrder?.order?.warehouse?.latitude},${singleReturnedOrder?.order?.warehouse?.longitude}`}
              target={"_blank"}
            >
              <LiaDirectionsSolid color="#070648" size={25} />{" "}
              <span className="ms-1">Direction</span>
            </a>
            {singleReturnedOrder?.order?.warehouse?.phone ? (
              <a
                href={`tel:${singleReturnedOrder?.order?.warehouse?.phone}`}
                className="call-button text-center mt-3 flex items-center justify-center no-underline"
              >
                <CiMobile2 color="#fff" size={24} />{" "}
                <span className="ms-1">Call</span>
              </a>
              ...
              <a
                className="call-button-disable text-center mt-3 flex items-center justify-center no-underline"
                title="Number Not Giver Sorry"
              >
                <CiMobile2 color="#fff" size={24} />{" "}
                <span className="ms-1">Call</span>
          ...
          <div className="leading-loose text-muted px-3 tracking-tighter">
            ...
            <div className="flex justify-between">
              ...
              <div className="font-semibold">
                {singleReturnedOrder?.quantity +
                  " x " +
                  parseFloat(singleReturnedOrder?.price)}
              </div>
            </div>
            {/* )} */}
            {singleReturnedOrder?.order_item_variants &&
              singleReturnedOrder.order_item_variants.map((variant) => (
            ...
            {singleReturnedOrder?.order_item_addons &&
              singleReturnedOrder.order_item_addons.map((variant) => (
                <div className="flex justify-between" key={variant.id}>
                  <div className="font-semibold">{variant.addon_name}</div>
                  <div className="font-semibold">
                    {variant.addon_quantity +
                      " x " +
                      parseFloat(variant.addon_price)}
                  </div>
                </div>
              ))}
            ...
            <div className="flex justify-between">
              ...
              <div className="font-bold">
                Rs {parseFloat(Returningamount?.returning_amount).toFixed(2)}
              </div>
            </div>
          </div>
        </div>
      ...
      <div
        className="px-3 w-100  mt-3 pb-8  bg-slate-50"
        style={{ bottom: "0px" }}
      >
        {singleOrder?.order_status_id == 2 ||
        singleOrder?.order_status_id == 3 ||
        singleOrder?.order_status_id == 4 ? (
                ...
                <Drawer.Content className="bg-zinc-100 flex flex-col rounded-t-[10px] mt-24 fixed bottom-0 left-0 right-0 h-80">
                  <div className=" bg-white rounded-t-[10px] flex-1  pt-4">
                    ...
                    <div className="fixed bottom-0 w-100 ">
                      <form
                        onSubmit={() =>
                          AcceptCancelBtn(singleOrder?.id, "cancel")
                        }
                      >
                  ...
                  <div className=" bg-white rounded-t-[10px] flex-1  pt-4">
                    ...
                    <div className="fixed bottom-0 w-100 ">
                      <div className="px-3 pb-4 flex items-center">
                        <div className="w-100 ">
                          <div className="text-center font-semibold text-lg pb-4 px-3">
                          ...
                          <button
                            ...
                            className="get-start-button w-100 text-xl"
                            onClick={() =>
                              AcceptCancelBtn(singleOrder?.id, "accept")
                            }
                          >
                            Accept Order
                          </button>
                ...
                <div className=" bg-white rounded-t-[10px] flex-1  pt-4">
                  ...
                  <div className="fixed bottom-0 w-100 ">
                    <div className="px-3 pb-4 flex items-center">
                      <div className="w-100">
                        <div className="text-center font-semibold text-lg pb-4 px-5">
                          Are you sure you want to confirm order pickup?
                        </div>
                        <button
                          type="submit"
                          className="get-start-button w-100 text-xl"
                          onClick={() => PickedupOrder(singleOrder?.id)}
                        >
                          Order Pickedup
                        </button>
                      </div>
                    ...
                    <div className="px-3 pb-4 flex items-center">
                      <div className="w-100">
                        ...
                        <button
                          type="submit"
                          className="get-start-button w-100 text-xl"
                          onClick={() => DeliveredOrder(singleOrder?.id)}
                        >
                          Order Delivered
                        </button>
                      </div>
