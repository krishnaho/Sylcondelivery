...
import {
  getPickedReturnUpOrders,
  getPickedUpOrders,
  orderDelivered,
  orderDeliveredReturned,
...
function PickedUpOrders() {
  ...
  const pickedUpOrders = useSelector(
    (state) => state.order?.pickedup_order?.data
  );
  const pickedUpReturnOrders = useSelector(
    (state) => state.order?.pickedup_return_order?.data
  );
  console.log(pickedUpReturnOrders, "pickedUpReturnOrders");
  ...
  useEffect(() => {
    if (
      ...
      dispatch(getPickedUpOrders(formData))
        .then((response) => {
  ...
  useEffect(() => {
    if (
      ...
      formData.append("token", localStorage.getItem("auth_token"));
      formData.append("user_id", localStorage.getItem("user_id"));
      dispatch(getPickedReturnUpOrders(formData))
        .then((response) => {
          if (response.payload.success) {
            setLoading(false);
          } else {
    ...
    dispatch(orderDelivered(formData))
      .then((response) => {
        if (response.payload.success) {
          ...
          dispatch(getPickedUpOrders(form_data))
            .then((response) => {
              if (response.payload.success) {
                setLoading(false);
              } else {
  ...
  const actionBtnReturned = (orderId) => {
    ...
    formData.append("user_id", localStorage.getItem("user_id"));
    formData.append("order_id", orderId);
    dispatch(orderDeliveredReturned(formData))
      .then((response) => {
        if (response.payload.success) {
          const form_data = new FormData();
          form_data.append("token", localStorage.getItem("auth_token"));
          form_data.append("user_id", localStorage.getItem("user_id"));
          dispatch(getPickedReturnUpOrders(form_data))
            .then((response) => {
              if (response.payload.success) {
                setLoading(false);
              } else {
    ...
    <div style={{ minHeight: "100vh" }} className="bg-slate-50">
      ...
      {loading ? (
        ...
        <>
         ...
         { pickedUpReturnOrders &&
                pickedUpReturnOrders?.pickedUpOrders && pickedUpReturnOrders?.pickedUpOrders?.length >0 &&
          ...
          <div className="px-3 w-100" style={{ paddingBottom: "8rem" }}>
            {
              pickedUpReturnOrders &&
                pickedUpReturnOrders?.pickedUpOrders &&
                pickedUpReturnOrders?.pickedUpOrders.map((order) => (
                  <div
                    className="bg-white py-3 rounded-xl mt-3"
                    style={{ boxShadow: "0px 0px 20px #99999938" }}
                  >
      ...
      { pickedUpReturnOrders &&
                pickedUpReturnOrders?.pickedUpOrders  &&  !pickedUpReturnOrders?.pickedUpOrders.length  && 
                 pickedUpOrders &&
      ...
      {open ? null : <Footer activePickedupOrder={true} />}
    </div>
  );
}
