...
import { getAcceptedOrders, pickedupOrder } from '../../../redux/store/orderReducer';
...
function AcceptedOrders() {
    ...
    const acceptedOrders = useSelector((state) => state.order?.accepted_order?.data);
    ...
    useEffect(() => {
        if (localStorage.getItem("userLogin") && localStorage.getItem("auth_token")) {
            ...
            dispatch(getAcceptedOrders(formData)).then((response) => {
    ...
    const actionBtn = (orderId) => {
        ...
        dispatch(pickedupOrder(formData)).then((response) => {
            if (response.payload.success) {
                ...
                dispatch(getAcceptedOrders(form_data)).then((response) => {
                    if (response.payload.success) {
                        setLoading(false)
                    } else {
                        setLoading(false)
        ...
        <div style={{ minHeight: "100vh" }} className='bg-slate-50'>
            ...
            {loading ?
                ...
                <div className='px-3 w-100' style={{ paddingBottom: "8rem" }}>
                    {acceptedOrders && acceptedOrders?.acceptedOrders && acceptedOrders?.acceptedOrders?.length > 0 ?
            ...
            {open ? null :
                <Footer activeAcceptedOrder={true} />
            }
